...
public class StreamsChallenge {
    ...
    public void executeChallenge() {
        //series.stream().map(Series::getEpisodes).reduce((a, b)->a+b);
      //float c= series.stream().map(Series::getEpisodes).reduce((a,b)->(a+b)/series.size()).get();




  //done        getEpisodesTotalCount();
   //done       getAverageEpisodesCount();
   //done       getMaxEpisodeCount();
...
//        getBestShounenStudio();
    ...
    private void getAverageEpisodesCount(){
        ...
        //series.stream().map(Series::getEpisodes).collect().forEach(System.out::println);
        average =  series.stream().map(Series::getEpisodes).reduce((a,b)->(a+b)/series.size()).get();
